...
def heatmap_plt(adata, 
  ...
  '''
  Draws co-expression scatter plot for given genes using given anndata object
  
  :param adata: single cell object to be plotted 
  :param meta_id: adata.obs column to plot heatmap over
  :param gene_list: list of genes to plot heatmap over
  :param use_raw: either to use raw gene counts
  :param layer: which layer to extract the gene expressions
  ...
  :param height: height of the plot. Can be auto or 'true_asp_ratio' or any value Plotly graph objects accepts. If set to true_asp_ratio, width must be explicit and height will be set using min/max values of dimention reduction axis values
  :param title_size: font size for title
  :param title: title for the plot
  :param cluster_rows: Either to cluster rows
  :param cluster_columns: Either to cluster columns
  ...
  fig = set_2d_layout(fig, ticks_font_size, 
                      dimred_labels = None,
                      axis_font_size = None,
                      legend_size = None,
