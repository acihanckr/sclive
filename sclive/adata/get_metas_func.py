...
def get_metas(adata, meta_ids, cat=None):
    '''
    Extract meta data from AnnData object and return as Polars DataFrame
    
    :param adata: AnnData object to extract meta data from    
    :param meta_ids: List of meta data ids to export from Annotated data
    :param cat: If True, then the meta data is assumed categorical, if false it is assumed numeric, if None, then the function will try to infer the type using pandas api

    Returns: 
    --------
    Polars DataFrame with meta data and barcodes
    '''
    if cat is None:
